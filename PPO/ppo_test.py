...
class Model(nn.Module):
    def __init__(self, obs_dim, act_dim, save_dir="./ppo_test_model"):
        super(Model, self).__init__()
        self.cnn_base = nn.Sequential(  
            nn.Conv2d(obs_dim, 16, kernel_size=3, stride=2),
            nn.ReLU(),
        ...
        self.v = nn.Sequential(nn.Linear(2592, 128), nn.ReLU(), nn.Linear(128, 1))
        self.fc = nn.Sequential(nn.Linear(2592, 128), nn.ReLU())
        self.alpha = nn.Sequential(nn.Linear(128, act_dim), nn.Softplus())
        self.beta = nn.Sequential(nn.Linear(128, act_dim),nn.Softplus())
...
class Agent:
    def __init__(self, state_dim, action_dim, gamma=0.99, lamda=0.95, clip=0.1,
                 learning_rate=1e-3, batch_size=128, save_dir='./ppo_test_model',
...
if __name__ == "__main__":
    ...
    if train:
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('ppo_test_training_score.png')
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('ppo_test_policy_loss.png')
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('ppo_test_value_loss.png')
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig('ppo_test_total_loss.png')


    else:
        print("... start testing ...")
        env = Env(render=True)
        agent = Agent(state_dim=3, action_dim=3, save_dir='./ppo_test_model')
        agent.load_model()
