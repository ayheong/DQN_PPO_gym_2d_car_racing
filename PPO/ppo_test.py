...
class Env:
    def __init__(self, sample_f = 1, render = False):
        if render: 
    ...
    def step(self, action):
        ...
        for i in range(self.sample_f):
            ...
            if done or done_ or np.mean(self.reward_queue) <= -0.1:
                done = True
                break
        state = state[:84, 6:90]
    ...
    def update_reward(self, r):
        ...
        assert len(self.reward_queue) == 100
...
class Agent:
    def __init__(self, state_dim, action_dim, gamma=0.99, lamda=0.95, clip=0.1,
                 learning_rate=0.001, batch_size=128, save_dir='./ppo_test_model',
...
def ppo_test(env, agent, n_episode=500):
    ...
    for episode in range(n_episode):
        ...
        print(f"Ep. {episode:04}, Ep.s {episode_steps:04}, Total.s {total_steps:07}, L.s {learn_steps:04},",
              f"R. {total_reward:1f}, Avg. {avg_score:1f}")

    return scores


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Car racing v2')
